...
const TabDescriptionMusic = ({
    ...
    listAudioBook.map((item) => (
      <SwiperSlide key={item.productId}>
        <div
          className=""
          key={item.productId}
          onClick={() => history.push(`${ROUTERS.MUSIC}/${item.productId}`)}
          role="button"
          tabIndex={0}
          onKeyDown={() => {}}
        >
  ...
  const handleSort = (types) => {
    const result = Immutable.asMutable(listReview);
    switch (types) {
      case 0:
...
export default memo<Props>(TabDescriptionMusic);
