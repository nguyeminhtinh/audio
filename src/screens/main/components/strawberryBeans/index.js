...
type Props = {
  ...
  getDataThemeLikeThis: Function,
  dataThemeLikeThis: Object,
...
const StrawberryBeans = ({
  ...
  const titleStrawberry = history?.location?.state?.title;
  const defaultOption = {
    listAudioSelective: [],
  };
  ...
  // call api theme like this
  useEffect(() => {
    getDataThemeLikeThis(themeId);
  }, [getDataThemeLikeThis, themeId]);
  ...
  const renderStrawberry =
    dataThemeLikeThis?.cases?.length > 0 ? (
      dataThemeLikeThis &&
      dataThemeLikeThis.cases &&
      dataThemeLikeThis.cases.map((item, index) => {
    ...
    >
      ...
      {isProcessing ? (
        ...
        <div className="strawberry">
          {titleStrawberry && (
            <h2 className="strawberry__title title-custom">
              {titleStrawberry}
            </h2>
          )}
          {dataThemeLikeThis?.cases?.length > 0 && (
            <div className="strawberry__top strawberry-dj-top">
              <div className="strawberry__top__total">
                ...
                {dataThemeLikeThis &&
                  dataThemeLikeThis.cases &&
                  dataThemeLikeThis.cases.length.toLocaleString('en')}
                개
              </div>
                ...
                <p
                  onClick={() => {
                    getListAudio(
                      dataThemeLikeThis &&
                        dataThemeLikeThis.cases &&
                        Immutable.asMutable(dataThemeLikeThis.cases)
                    );
                    setListAudio([]);
...
export default memo<Props>(StrawberryBeans);
